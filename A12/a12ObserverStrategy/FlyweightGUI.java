...
class FlyweightGUI {
	...
	public static void main( String[] args ) {
		...
		frame.setLayout( new GridLayout( NUM, RAN ) );
		int startInt = 0;
		for (int i=0; i < NUM; i++)
			for (int j=0; j < RAN; j++){
		...
		LocalObserver local = new LocalObserver(ChainFactory.getChain().s);
		PrimeObserverProxy remotePrime = new PrimeObserverProxy(ChainFactory.getChain().s);
		CountingObserverProxy remoterCounter = new CountingObserverProxy(ChainFactory.getChain().s);
	}  
}
